...
create database [SQLcompliance.Processing]
GO

ALTER DATABASE [SQLcompliance.Processing] SET RECOVERY SIMPLE
GO

CREATE TABLE [SQLcompliance.Processing]..[TraceTimes] (
...
CREATE TABLE [SQLcompliance.Processing]..[TraceStates] (
	[instance] [nvarchar] (128) NULL ,
	[traceType] [int] NULL ,
	[state] [int] NULL ,
	[updated] [datetime] NULL ,
...
as
select '<ALL>' as instance UNION select instance from Servers WITH (NOLOCK) order by instance
GO
...
IF NOT EXISTS (SELECT * FROM sys.indexes WHERE object_id = OBJECT_ID(N'[dbo].[Alerts]') AND name = N'IX_Alerts_instance_alertLevel_isDismissed_created')
	CREATE INDEX [IX_Alerts_instance_alertLevel_isDismissed_created] 
	ON [dbo].[Alerts] 
	( [instance], [alertLevel], [isDismissed] )
	INCLUDE( [created] )
...
BEGIN
	...
	EXEC(N'INSERT INTO [EventTypes] ([evtypeid],[evcatid],[name],[category],[is2005Only],[isExcludable])	VALUES (1810,1,''Log out'',''Login'',0,1)');
