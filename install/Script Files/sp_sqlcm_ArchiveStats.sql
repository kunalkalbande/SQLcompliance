USE [SQLcompliance]
GO
/****** Object:  StoredProcedure [dbo].[sp_sqlcm_ArchiveStats]    Script Date: 9/21/2016 2:26:10 PM ******/
...
IF (OBJECT_ID('sp_sqlcm_ArchiveStats') IS NOT NULL)
BEGIN
	DROP PROCEDURE sp_sqlcm_ArchiveStats
END
GO

Create procedure [dbo].[sp_sqlcm_ArchiveStats] (
...
declare @conflictPKCount as int
begin
...
if (OBJECT_ID( '['+@archiveDbName+'].[dbo].[Stats]' ) IS NULL )
begin
set @dbName=' Create table ['+@archiveDbName+'].[dbo].[Stats]'
Set @sql=''
set @sql='
	( 
...
set @sql=''
set @dbName=''
set @serverName='[SQLcompliance_'+@InstanceDatabase+'].[dbo].[Stats]'
set @sql = 'SELECT * FROM ' + @serverName + ' st INNER JOIN [' +@archiveDbName + '].[dbo].[Stats] ast ON ast.dbId=st.dbId and ast.date=st.date and ast.category=st.category';
exec (@sql);
select @conflictPKCount = @@rowcount;
if(@conflictPKCount > 0)
begin
  set @sql = '';
  set @sql = 'UPDATE ast SET [count] = ([st].[count] + [ast].[count]), lastUpdated = st.lastUpdated FROM ['+@archiveDbName+
  + '].[dbo].[Stats] ast INNER JOIN ' + @serverName + 'st ON (ast.dbId = st.dbId and ast.date = st.date and ast.category = st.category);'
  + ' DELETE st FROM ' + @serverName + ' st INNER JOIN ['+@archiveDbName+ 
  +	'].[dbo].[Stats] ast ON (ast.dbId = st.dbId and ast.date = st.date and ast.category = st.category);'
  exec (@sql);
end
set @sql='INSERT INTO ['+@archiveDbName+'].[dbo].[Stats] '
set @sql=@sql + '([dbId], [date], [category], [count], [lastUpdated]) SELECT dbId, date, category, count, lastUpdated FROM '
set @type=UPPER(@type)
if(@type in ('JAN','FEB','MAR','APR','MAY','JUN','JUL','AUG','SEP','OCT','NOV','DEC'))
begin
		...
		set @sql =@sql + @serverName +' where month(lastUpdated)= '+CONVERT(varchar(10), @currenMonth) +' and Day(lastUpdated)<= '+CONVERT(varchar(10), @remainDay) +' and Year(lastUpdated) <= '+CONVERT(varchar(10), @currenYear)
		end
		else
		begin
		
		set @sql =@sql + @serverName +' where month(lastUpdated) = '+CONVERT(varchar(10), @currenMonth) +' and Year( lastUpdated )<= '+CONVERT(varchar(10), @currenYear)
...
begin
		...
		set @sql =@sql + @serverName +' where month(lastUpdated) >= '+CONVERT(varchar(10), @first) +' and month(lastUpdated) <= '+CONVERT(varchar(10), @last) 
		set @sql =@sql +' and CONVERT(varchar(10), cast ([lastUpdated] as datetime)) <=' 
		set @sql =@sql+ @remianDays +' and Year(lastUpdated) <= '
		...
		begin
		set @sql =@sql + @serverName +' where month(lastUpdated) >= '+CONVERT(varchar(10), @first) +' and month(lastUpdated) <= '+CONVERT(varchar(10), @last) +' and Year( lastUpdated ) <= '+CONVERT(varchar(10), @currenYear)
...
begin
	...
	if(convert(varchar(10),@currenYear) = @currentYear)
		begin
		set @sql =@sql + @serverName +' where Year( lastUpdated ) <= '+CONVERT(varchar(10), @currenYear)
		set @sql =@sql +' and CONVERT(varchar(10), cast ([lastUpdated] as datetime)) <= '+ @remianDays
...
begin
	if(@currentMonth = @type)
		begin
			set @sql=''
			set @sql ='DELETE FROM '+ '[SQLcompliance_'+@InstanceDatabase+'].[dbo].[Stats] where month(lastUpdated)= '+CONVERT(varchar(10), @currenMonth) +' and Day(lastUpdated)<= '+CONVERT(varchar(10), @remainDay) +' and Year(lastUpdated) <= '+CONVERT(varchar(10), @currenYear)
			exec(@sql)	
		end
	else
		begin
			set @sql=''
			set @sql ='DELETE FROM '+ '[SQLcompliance_'+@InstanceDatabase+'].[dbo].[Stats] where month(lastUpdated) = '+CONVERT(varchar(10), @currenMonth) +' and Year( lastUpdated )<= '+CONVERT(varchar(10), @currenYear)
			exec(@sql)	
		end
...
begin
	set @remianDays= ''''+CONVERT(varchar(11), cast (DATEADD(day, -@days, GETDATE()) AS datetime))+''''
		...
		begin
			set @sql=''
			set @sql ='DELETE FROM '+ '[SQLcompliance_'+@InstanceDatabase+'].[dbo].[Stats]'
			set @sql =@sql +' where month(lastUpdated) >= '+CONVERT(varchar(10), @first) +' and month(lastUpdated) <= '+CONVERT(varchar(10), @last) 
			set @sql =@sql +' and CONVERT(varchar(10), cast ([lastUpdated] as datetime)) <=' 
			set @sql =@sql+ @remianDays +' and Year(lastUpdated) <= '
			set @sql =@sql +CONVERT(varchar(10), @currenYear)
		...
		begin
			set @sql=''
			set @sql ='DELETE FROM '+ '[SQLcompliance_'+@InstanceDatabase+'].[dbo].[Stats]'
			set @sql =@sql +' where month(lastUpdated) >= '+CONVERT(varchar(10), @first) +' and month(lastUpdated) <= '+CONVERT(varchar(10), @last) +' and Year( lastUpdated ) <= '+CONVERT(varchar(10), @currenYear)
	...
	if(convert(varchar(10),@currenYear) = @currentYear)
		begin
			set @sql=''
			set @sql ='DELETE FROM '+ '[SQLcompliance_'+@InstanceDatabase+'].[dbo].[Stats]'
			set @sql =@sql +' where Year( lastUpdated ) <= '+CONVERT(varchar(10), @currenYear)
			set @sql =@sql +' and CONVERT(varchar(10), cast ([lastUpdated] as datetime)) <= '+ @remianDays
