USE [SQLcompliance]
GO
/****** Object:  StoredProcedure [dbo].[sp_sqlcm_ArchiveColumnChanges]    Script Date: 9/21/2016 2:26:10 PM ******/
...
IF (OBJECT_ID('sp_sqlcm_ArchiveColumnChanges') IS NOT NULL)
BEGIN
	DROP PROCEDURE sp_sqlcm_ArchiveColumnChanges
END
GO
Create procedure [dbo].[sp_sqlcm_ArchiveColumnChanges] (
...
if (OBJECT_ID( '['+@archiveDbName+'].[dbo].[ColumnChanges]' ) IS NULL )
begin
set @dbName=' Create table ['+@archiveDbName+'].[dbo].[ColumnChanges]'
Set @sql=''
set @sql='
	( 
...
set @sql=''
set @dbName=''
set @serverName='[SQLcompliance_'+@InstanceDatabase+'].[dbo].[ColumnChanges]'
set @sql='INSERT INTO ['+@archiveDbName+'].[dbo].[ColumnChanges] '
set @sql=@sql + '([startTime], [eventSequence], [spid], [columnName], [beforeValue], [afterValue], [hashcode], [columnId],  [dcId], [ccId]) 
SELECT [startTime], [eventSequence], [spid], [columnName], [beforeValue], [afterValue], [hashcode], [columnId],  [dcId], [ccId] FROM '
set @type=UPPER(@type)
if(@type in ('JAN','FEB','MAR','APR','MAY','JUN','JUL','AUG','SEP','OCT','NOV','DEC'))
begin
...
begin
	if(@currentMonth = @type)
		begin
			set @sql=''
			set @sql ='DELETE FROM '+ '[SQLcompliance_'+@InstanceDatabase+'].[dbo].[ColumnChanges] where month(startTime)= '+CONVERT(varchar(10), @currenMonth) +' and Day(startTime)<= '+CONVERT(varchar(10), @remainDay) +' and Year(startTime) <= '+CONVERT(varchar(10), @currenYear)
			exec(@sql)	
		end
	else
		begin
			set @sql=''
			set @sql ='DELETE FROM '+ '[SQLcompliance_'+@InstanceDatabase+'].[dbo].[ColumnChanges]  where month(startTime) = '+CONVERT(varchar(10), @currenMonth) +' and Year( startTime )<= '+CONVERT(varchar(10), @currenYear)
			exec(@sql)	
		end
...
begin
	set @remianDays= ''''+CONVERT(varchar(11), cast (DATEADD(day, -@days, GETDATE()) AS datetime))+''''
		...
		begin
			set @sql=''
			set @sql ='DELETE FROM '+ '[SQLcompliance_'+@InstanceDatabase+'].[dbo].[ColumnChanges]'
