...
IF (OBJECT_ID('sp_sqlcm_GetArchivedEvents') IS NOT NULL)
BEGIN
	DROP PROCEDURE sp_sqlcm_GetArchivedEvents
END
GO

CREATE PROCEDURE [dbo].[sp_sqlcm_GetArchivedEvents]
 @ServerId int,
...
INSERT INTO 
	#TempDatabases (eventDatabase)
SELECT 
	sd.databaseName
FROM 
	[SQLcompliance].[dbo].[SystemDatabases] sd
INNER JOIN [SQLcompliance].[dbo].[Servers] s ON s.instance = sd.instance
WHERE 
	sd.databaseType = 'Archive' 
	AND s.srvId = @ServerId 
AND (@Archive IS NULL OR sd.databaseName = @Archive)

DECLARE @NumberOfRecords INT;
SET @NumberOfRecords = (SELECT COUNT(*) FROM #TempDatabases)

WHILE (@Iterator <= @NumberOfRecords)
BEGIN
	DECLARE @EventDatabaseName nvarchar(128);
	SELECT @EventDatabaseName = eventDatabase FROM #TempDatabases WHERE RowID = @Iterator
...
WHERE 
(@DatabaseId IS NULL OR ( cDb.dbId = @DatabaseId AND cDb.name COLLATE DATABASE_DEFAULT = e.databaseName COLLATE DATABASE_DEFAULT ) )
...
EXEC sp_executesql @query, N'@EventDatabaseName nvarchar(128), @DatabaseId int, @TimeFromInSeconds bigint, @TimeToInSeconds bigint, @DateFrom datetime, @DateTo datetime, @EventCategoryId int',  
@EventDatabaseName, @DatabaseId, @TimeFromInSeconds, @TimeToInSeconds, @DateFrom, @DateTo, @EventCategoryId;
