...
if (@eventDatabase = '<ALL>')
	...
	declare @eventDatabaseName nvarchar(128),
		...
		if (object_id(N'tempdb..#loginCreationHistory') IS NOT NULL)
			drop table #loginCreationHistory
			
		create table #loginCreationHistory (
	...
	begin
	   ...
	   set @eventDatabaseName = dbo.fn_cmreport_ProcessString(@eventDatabaseName);
		...
		set @columns = 'Select TOP ' + STR(@rowCount) + ' * FROM ( select applicationName, hostName, CASE eventType WHEN 717 THEN targetLoginName WHEN 710 then targetLoginName ELSE targetObject END as ''targetLoginName'', loginName, startTime '
	...
	set @stmt = @columns + @fromClause + @whereClause + @tempTable + @whereDate + @whereTime + @orderByClause;
		...
		insert into #loginCreationHistory exec (@stmt) 
		...
		select * from #loginCreationHistory
	end
	else
	begin
