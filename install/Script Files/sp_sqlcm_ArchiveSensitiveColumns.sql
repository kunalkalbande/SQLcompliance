USE [SQLcompliance]
GO
/****** Object:  StoredProcedure [dbo].[sp_sqlcm_ArchiveSensitiveColumns]    Script Date: 9/21/2016 2:26:10 PM ******/
...
IF (OBJECT_ID('sp_sqlcm_ArchiveSensitiveColumns') IS NOT NULL)
BEGIN
	DROP PROCEDURE sp_sqlcm_ArchiveSensitiveColumns
END
GO

Create procedure [dbo].[sp_sqlcm_ArchiveSensitiveColumns] (
...
if (OBJECT_ID( '['+@archiveDbName+'].[dbo].[SensitiveColumns]' ) IS NULL )
begin
set @dbName=' Create table ['+@archiveDbName+'].[dbo].[SensitiveColumns] '
Set @sql=''
set @sql='
	( 
...
set @sql=''
set @dbName=''
set @serverName='[SQLcompliance_'+@InstanceDatabase+'].[dbo].[SensitiveColumns]'
set @sql='INSERT INTO ['+@archiveDbName+'].[dbo].[SensitiveColumns] '
set @sql=@sql + '(startTime, eventId, columnName, hashcode, tableId, columnId, scId) SELECT startTime, eventId, columnName, hashcode, tableId, columnId, scId FROM '
set @type=UPPER(@type)
if(@type in ('JAN','FEB','MAR','APR','MAY','JUN','JUL','AUG','SEP','OCT','NOV','DEC'))
begin
...
begin
	if(@currentMonth = @type)
		begin
			set @sql=''
			set @sql ='DELETE FROM '+ '[SQLcompliance_'+@InstanceDatabase+'].[dbo].[SensitiveColumns] where month(startTime)= '+CONVERT(varchar(10), @currenMonth) +' and Day(startTime)<= '+CONVERT(varchar(10), @remainDay) +' and Year(startTime) <= '+CONVERT(varchar(10), @currenYear)
			exec(@sql)	
		end
	else
		begin
			set @sql=''
			set @sql ='DELETE FROM '+ '[SQLcompliance_'+@InstanceDatabase+'].[dbo].[SensitiveColumns] where month(startTime) = '+CONVERT(varchar(10), @currenMonth) +' and Year( startTime )<= '+CONVERT(varchar(10), @currenYear)
			exec(@sql)	
		end
...
begin
	set @remianDays= ''''+CONVERT(varchar(11), cast (DATEADD(day, -@days, GETDATE()) AS datetime))+''''
		...
		begin
			set @sql=''
			set @sql ='DELETE FROM '+ '[SQLcompliance_'+@InstanceDatabase+'].[dbo].[SensitiveColumns]'
