USE [SQLcompliance]
GO
/****** Object:  StoredProcedure [dbo].[sp_sqlcm_ArchiveEventSQL]    Script Date: 9/21/2016 2:26:10 PM ******/
...
GO
IF (OBJECT_ID('sp_sqlcm_ArchiveEventSQL') IS NOT NULL)
BEGIN
	DROP PROCEDURE sp_sqlcm_ArchiveEventSQL
END
GO

Create procedure [dbo].[sp_sqlcm_ArchiveEventSQL] (
...
if (OBJECT_ID( '['+@archiveDbName+'].[dbo].[EventSQL]' ) IS NULL )
begin
set @dbName=' Create table ['+@archiveDbName+'].[dbo].[EventSQL] '
Set @sql=''
set @sql='
...
set @sql=''
set @dbName=''
set @serverName='[SQLcompliance_'+@InstanceDatabase+'].[dbo].[EventSQL]'
set @sql='INSERT INTO ['+@archiveDbName+'].[dbo].[EventSQL] '
set @sql=@sql + '([eventId],[startTime],[sqlText],[hash]) SELECT [eventId],[startTime],[sqlText],[hash] FROM '
set @type=UPPER(@type)
if(@type in ('JAN','FEB','MAR','APR','MAY','JUN','JUL','AUG','SEP','OCT','NOV','DEC'))
begin
...
begin
	if(@currentMonth = @type)
		begin
			set @sql=''
			set @sql ='DELETE FROM '+ '[SQLcompliance_'+@InstanceDatabase+'].[dbo].[EventSQL] where month(startTime)= '+CONVERT(varchar(10), @currenMonth) +' and Day(startTime)<= '+CONVERT(varchar(10), @remainDay) +' and Year(startTime) <= '+CONVERT(varchar(10), @currenYear)
			exec(@sql)	
		end
	else
		begin
			set @sql=''
			set @sql ='DELETE FROM '+ '[SQLcompliance_'+@InstanceDatabase+'].[dbo].[EventSQL] where month(startTime) = '+CONVERT(varchar(10), @currenMonth) +' and Year( startTime )<= '+CONVERT(varchar(10), @currenYear)
			exec(@sql)	
		end
...
begin
	set @remianDays= ''''+CONVERT(varchar(11), cast (DATEADD(day, -@days, GETDATE()) AS datetime))+''''
		...
		begin
			set @sql=''
			set @sql ='DELETE FROM '+ '[SQLcompliance_'+@InstanceDatabase+'].[dbo].[EventSQL]'
