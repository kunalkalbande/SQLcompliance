USE [SQLcompliance]
GO
/****** Object:  StoredProcedure [dbo].[sp_sqlcm_ArchiveLogins]    Script Date: 2/19/2018 11:19:03 AM ******/
SET ANSI_NULLS OFF
GO
SET QUOTED_IDENTIFIER OFF
GO
IF (OBJECT_ID('sp_sqlcm_ArchiveLogins') IS NOT NULL)
BEGIN
	DROP PROCEDURE sp_sqlcm_ArchiveLogins
END
GO

create procedure [dbo].[sp_sqlcm_ArchiveLogins] (
...
if (OBJECT_ID( '['+@archiveDbName+'].[dbo].[Logins]' ) IS NULL )
begin
set @dbName=' Create table ['+@archiveDbName+'].[dbo].[Logins] '
Set @sql=''
set @sql='( [name] [nvarchar] (128) NOT NULL,[id] [int] NOT NULL )'
...
set @sql=''
set @dbName=''
set @serverName='[SQLcompliance_'+@InstanceDatabase+'].[dbo].[Logins]'
set @sql='INSERT INTO ['+@archiveDbName+'].[dbo].[Logins] '
set @sql=@sql + '(name,id) ( SELECT l.name,l.id FROM  '+@serverName+' l LEFT JOIN '+'['+@archiveDbName+'].[dbo].[Logins] la ON l.name=la.name AND l.id=la.id where la.name IS NULL)'
print @sql;
set @type=UPPER(@type)
begin
exec (@sql);
