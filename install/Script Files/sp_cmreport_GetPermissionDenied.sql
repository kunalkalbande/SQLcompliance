...
declare @columns nvarchar(2000), @fromClause nvarchar(500), @whereClause nvarchar(MAX), @orderByClause nvarchar(200), @category nvarchar(200)  
...
begin 
	declare @eventDatabaseName nvarchar(128),
		...
		if (object_id(N'tempdb..#permissionDenied') IS NOT NULL)
			drop table #permissionDenied
	...
	set @stmt = @columns + @fromClause + @whereClause + @tempTable + @whereDate + @whereTime + @orderByClause;
		...
		insert into #permissionDenied exec (@stmt) 
