...
IF (OBJECT_ID('sp_sqlcm_ArchiveColumns') IS NOT NULL)
BEGIN
	DROP PROCEDURE sp_sqlcm_ArchiveColumns
END
GO

create procedure [dbo].[sp_sqlcm_ArchiveColumns] (
...
if (OBJECT_ID( '['+@archiveDbName+'].[dbo].[Columns]' ) IS NULL )
begin
set @dbName=' Create table ['+@archiveDbName+'].[dbo].[Columns] '
Set @sql=''
