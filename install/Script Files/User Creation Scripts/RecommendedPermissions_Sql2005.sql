/****** RecommendedPermissions_Sql2005.sql ******/
...
--
-- Permissions Check Script for SQL Server 2005
...
GO
	...
	END
GO
-- Provides Access to all the databases
...
BEGIN TRY
	...
	EXECUTE sp_addrolemember db_accessadmin, ''DOMAIN\USER''
	EXECUTE sp_addrolemember db_datareader, ''DOMAIN\USER''
	EXECUTE sp_addrolemember db_datawriter, ''DOMAIN\USER''
	EXECUTE sp_addrolemember db_ddladmin, ''DOMAIN\USER''
	EXECUTE sp_addrolemember db_owner, ''DOMAIN\USER''
	EXECUTE sp_addrolemember db_securityadmin, ''DOMAIN\USER''
    END
END TRY
BEGIN CATCH
  -- Empty Catch
...
EXECUTE sp_addsrvrolemember 'DOMAIN\USER', securityadmin
EXECUTE sp_addsrvrolemember 'DOMAIN\USER', dbcreator
...
';
	...
	DECLARE @Cnt INT;
	SELECT @Cnt = COUNT(*) FROM sys.fn_my_permissions(NULL,NULL) where permission_name IN('ALTER ANY DATABASE', 'ALTER ANY EVENT NOTIFICATION', 'ALTER ANY LOGIN', 'ALTER RESOURCES', 'ALTER SERVER STATE', 'ALTER SETTINGS', 'ALTER TRACE', 'CONNECT SQL', 'CONTROL SERVER', 'CREATE ANY DATABASE', 'VIEW ANY DATABASE', 'VIEW ANY DEFINITION', 'VIEW SERVER STATE');
	IF(@Cnt <> 13)
	BEGIN
		SELECT 0;
		RETURN;
