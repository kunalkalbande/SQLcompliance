USE [SQLcompliance]
GO
/****** Object:  StoredProcedure [dbo].[sp_sqlcm_ArchiveDataChanges]    Script Date: 9/21/2016 2:26:10 PM ******/
...
GO
IF (OBJECT_ID('sp_sqlcm_ArchiveDataChanges') IS NOT NULL)
BEGIN
	DROP PROCEDURE sp_sqlcm_ArchiveDataChanges
END
GO
Create procedure [dbo].[sp_sqlcm_ArchiveDataChanges] (
...
--set @currentDay=DAY(GETDATE()) -@days
set @type = UPPER(@type)
if(@days > 0)
BEGIN
...
if (OBJECT_ID( '['+@archiveDbName+'].[dbo].[DataChanges]' ) IS NULL )
begin
set @dbName=' Create table ['+@archiveDbName+'].[dbo].[DataChanges]'
Set @sql=''
set @sql='
	( 
...
set @sql=''
set @dbName=''
set @serverName='[SQLcompliance_'+@InstanceDatabase+'].[dbo].[DataChanges]'
set @sql='INSERT INTO ['+@archiveDbName+'].[dbo].[DataChanges] '
set @sql=@sql + '(startTime,eventSequence,spid,databaseId,actionType,schemaName,tableName,recordNumber,userName,changedColumns,primaryKey,hashcode,tableId,dcId,eventId,totalChanges) SELECT startTime,eventSequence,spid,databaseId,actionType,schemaName,tableName,recordNumber,userName,changedColumns,primaryKey,hashcode,tableId,dcId,eventId,totalChanges FROM '
set @type=UPPER(@type)
if(@type in ('JAN','FEB','MAR','APR','MAY','JUN','JUL','AUG','SEP','OCT','NOV','DEC'))
begin
...
begin
	if(@currentMonth = @type)
		begin
			set @sql=''
			set @sql ='DELETE FROM '+ '[SQLcompliance_'+@InstanceDatabase+'].[dbo].[DataChanges] where month(startTime)= '+CONVERT(varchar(10), @currenMonth) +' and Day(startTime)<= '+CONVERT(varchar(10), @remainDay) +' and Year(startTime) <= '+CONVERT(varchar(10), @currenYear)
			exec(@sql)	
		end
	else
		begin
			set @sql=''
			set @sql ='DELETE FROM '+ '[SQLcompliance_'+@InstanceDatabase+'].[dbo].[DataChanges]  where month(startTime) = '+CONVERT(varchar(10), @currenMonth) +' and Year( startTime )<= '+CONVERT(varchar(10), @currenYear)
			exec(@sql)	
		end
...
begin
	set @remianDays= ''''+CONVERT(varchar(11), cast (DATEADD(day, -@days, GETDATE()) AS datetime))+''''
		...
		begin
			set @sql=''
			set @sql ='DELETE FROM '+ '[SQLcompliance_'+@InstanceDatabase+'].[dbo].[DataChanges]'
