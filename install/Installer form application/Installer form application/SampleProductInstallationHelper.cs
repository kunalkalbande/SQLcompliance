...
namespace Installer_form_application
{
    class SampleProductInstallationHelper
    {
        ...
        {
            url = url + "/IderaCoreServices/v1/Products/IsAvailable?productname=SampleProduct&instancename=" + instanceName + "&version=1.0.0.0";
            Response response = GetRequest(url, username, password);
            bool instanceExists = false;
            if (response.httpStatusCode != HttpStatusCode.OK)
                instanceExists = instanceName == "firstinstance" ? true : false;
        ...
        {
            // this web request gets all the products intalled with shortname SampleProduct
            url = url + "/IderaCoreServices/v1/Products/SampleProduct";
            Response response = GetRequest(url, username, password);
            if (response.httpStatusCode == HttpStatusCode.OK)
            {
