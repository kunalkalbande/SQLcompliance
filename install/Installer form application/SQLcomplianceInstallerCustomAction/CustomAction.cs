...
namespace SQLcomplianceInstallerCustomAction
{
    public static class CustomActions
    {
        ...
        static string GetConnectionString(Session session)
        {
            ...
            connectionStringBuilder.IntegratedSecurity = session[SqlServerAuthentication].Equals("0");
            ...
            connectionStringBuilder.UserID = session[SqlServerUser];
            connectionStringBuilder.Password = session[SqlServerPassword];
        ...
        private static int GetMoniteredServerCount(Session session)
        {
            ...
            using (var connection = new SqlConnection(GetConnectionString(session)))
            {
                try
                {
                    connection.Open();
