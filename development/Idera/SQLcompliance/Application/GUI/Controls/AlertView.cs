...
{
    public delegate void VoidDelegateListAlerts(List<Alert> alerts);

    public partial class AlertView : BaseControl
    {
        ...
        public AlertViewFilter Filter
      {
        ...
        private void DoWork_bgAlertLoader(object sender, DoWorkEventArgs e)
        {
            ...
            try
            {
                alerts = AlertingDal.SelectAlerts(query, _configuration.ConnectionString);
            }
            catch (Exception ex)
            {
        ...
        private void RunWorkerCompleted_bgAlertLoader(object sender, RunWorkerCompletedEventArgs e)
        {
            if (InvokeRequired)
            {
                ...
                Invoke(new VoidDelegateListAlerts(PostLoadAlertPage), parameters);
            }
            else
            {
                PostLoadAlertPage(e.Result as List<Alert>);
            }
        }

        private void PostLoadAlertPage(List<Alert> alerts)
        {
            ...
            try
            {
                ...
                else if (alerts.Count == 0)
                {
                    ...
                    _lblStatus.Text = UIConstants.Grid_NoAlerts;
                    _lblNavigation.Value = string.Format("<b>{0}</b>", UIConstants.Grid_NoAlerts);
                    _lblStatus.Visible = true;
                }
        ...
        private void UpdateRowValues(UltraDataRow row, Alert alert)
        {
            switch (alert.Level)
            {
            ...
            CMEventType evType = _configuration.LookupEventType(alert.EventType, alert.AlertType);
            row["eventTypeString"] = (evType == null) ? alert.EventType.ToString() : evType.ToString();
        ...
        public int Next(out string eventDatabase)
        {
            ...
            Alert activeAlert = GetActiveAlert();
            if (activeAlert != null)
            {
        ...
        public Alert GetActiveAlert()
        {
            ...
            else
            {
                ...
                return dataRow != null ? dataRow.Tag as Alert : null;
            }
        }
        ...
        private string BuildPageQuery()
        {
            ...
            whereClause = _filter.GetWhereClause(EventType.SqlServer);

            // Make sure we aren't the first page being loaded
            //  
            if (_pageStarts.Count > 0)
            {
                Alert pageStart = (Alert)_pageStarts.Peek();
            ...
            whereClause = string.Format("{0} AND alertType = 1", whereClause);

            return string.Format("SELECT TOP {0} alertId,alertType,alertRuleId,alertEventId," +
               "instance,eventType,created,alertLevel,emailStatus,logStatus,message,ruleName" +
               " FROM {1}..{2} WHERE {3} {4}",
        ...
        private void UpdateAlertCount()
        {
            ...
            string whereClause = _filter.GetWhereClause(EventType.SqlServer);
        ...
        {
            HelpAlias.ShowHelp(this, HelpAlias.SSHELP_AlertView);
        }
        ...
        /*
        private void Opening_cmAlerts(object sender, CancelEventArgs e)
        {
           Alert alert = GetActiveAlert();

           if (alert != null)
           {
              _miEventProperties.Enabled = true;
              if (alert.EmailStatus == NotificationStatus.None && alert.LogStatus == NotificationStatus.None)
                 _miAlertMessage.Enabled = false;
              else
                 _miAlertMessage.Enabled = true;
           }
           else
           {
              _miAlertMessage.Enabled = false;
              _miEventProperties.Enabled = false;
           }
           _miViewGroupBy.Checked = Globals.ViewGroupByColumn;
           _miExpand.Enabled = Globals.ViewGroupByColumn;
           _miCollapse.Enabled = Globals.ViewGroupByColumn;
        }

        private void Click_miCollapse(object sender, EventArgs e)
        {
           CollapseAll();
        }

        private void Click_miExpand(object sender, EventArgs e)
        {
           ExpandAll();
        }
        ...
        {
           Globals.ViewGroupByColumn = !Globals.ViewGroupByColumn;
           ShowGroupBy = Globals.ViewGroupByColumn;
        }*/
