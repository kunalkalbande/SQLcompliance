...
namespace Idera.SQLcompliance.Core.TimeZoneHelper
{
	...
	public class TimeZoneInfo : IComparable
	{
		      ...
		      for (int i = 0; i < zones.Length; i++)
		      {
		         ...
		         try
		         {
		            ...
		            try
		            {
		               ...
		               display = zoneKey.GetValue("Display") as string;
		               sName = zoneKey.GetValue("Std") as string;
		               dName = zoneKey.GetValue("Dlt") as string;
		            ...
		            if (display == null || sName == null || dName == null)
		               continue;
		            if (bytes == null)
		               continue;
		         ...
		         {
		            // Catch all to avoid problems here.
		            ErrorLog.Instance.Write("Error gathering timezone information.", e);
		         }
		      }
		      tzInfos = new TimeZoneInfo[timeZones.Count];
		      for (int i = 0; i < timeZones.Count; i++)
		      {
		         tzInfos[i] = (TimeZoneInfo) timeZones[i];
		      }
            ...
            out SystemTime     utcTime )
      {
		...
		{
		   if ( sourceLocalTime == DateTime.MinValue ) return sourceLocalTime;
