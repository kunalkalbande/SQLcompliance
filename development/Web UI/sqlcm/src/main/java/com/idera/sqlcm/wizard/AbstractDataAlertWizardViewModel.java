...
public abstract class AbstractDataAlertWizardViewModel<WSE extends IWizardEntity> {

	private static final Logger logger = Logger
			.getLogger(AbstractDataAlertWizardViewModel.class);
	...
	private WizardDataStepManager stepManager;
	...
	protected WizardDataStepManager createStepManager() {
		return new WizardDataStepManager(this);
	}
	...
	public abstract void registerSteps(final WizardDataStepManager stepManager);
	...
	public void afterCompose(@ContextParam(ContextType.VIEW) Component view,
		...
		IDataStep step = stepManager.getFirst();
		tips = step.getTips();
	...
	public final void close() {
		...
		if((String)Sessions.getCurrent().getAttribute("appMatchString")!=null){
    		Sessions.getCurrent().removeAttribute("appMatchString");
    		
    	} 
		if((String)Sessions.getCurrent().getAttribute("loginMatchString")!=null){
    		Sessions.getCurrent().removeAttribute("loginMatchString");
    	} 
		if((String)Sessions.getCurrent().getAttribute("rowCountDetails")!=null){
			Sessions.getCurrent().removeAttribute("rowCountDetails");
    	} 
		if(Sessions.getCurrent().getAttribute("isInitializedDataRule") !=null)
		{
			Sessions.getCurrent().removeAttribute("isInitializedDataRule");
		}
	...
	public void nextButtonClick() {
		if (stepManager.getCurrentStep().isValid()) {
			...
			if(Sessions.getCurrent().getAttribute("isValidSnmpAddress")==null 
				...
				IDataStep step = stepManager.next();
				updateTips(step);
	...
	private void updateButtonsVisibility(IDataStep step) {
		if (step == null) {
	...
	public void prevButtonClick() {
		...
		IDataStep step = stepManager.prev();
		updateTips(step);
		updateButtonsVisibility(step);
		step.onShow(wizardSaveEntity);
	}

	private void updateTips(IDataStep step) {
	...
	public IDataStep getCurrentStep() {
