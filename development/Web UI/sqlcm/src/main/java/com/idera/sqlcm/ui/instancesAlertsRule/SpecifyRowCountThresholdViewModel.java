...
public class SpecifyRowCountThresholdViewModel  {
	...
	public void GetData() {
		...
		if (!strMatchString.isEmpty()) {
			...
			if (EventNodeDataValue.containsKey("timeframe")) {
				if (!EventNodeDataValue.get("timeframe").toString().isEmpty())
					setTimeInterval(EventNodeDataValue.get("timeframe")
							.toString());
			}
		}
	}
	...
	public void submitChoice(@BindingParam("rowCountDropDown") Combobox cb,
		...
		Sessions.getCurrent().setAttribute("rowCountDetails", rowCountMatchString);
		x.detach();
	}
