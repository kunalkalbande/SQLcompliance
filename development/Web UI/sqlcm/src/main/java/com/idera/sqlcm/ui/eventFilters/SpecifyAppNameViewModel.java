...
public class SpecifyAppNameViewModel implements
	...
	public static enum Category {
		LISTED(1, ELFunctions.getLabel(SQLCMI18NStrings.LISTED)), 
	...
	public void selectAddEventFilter(
		...
		if (selectedIntervals != null && !selectedIntervals.isEmpty()) {
			for (Category i : selectedIntervals) {
				currentInterval = i;
				Sessions.getCurrent().setAttribute("specifyAppRadio",
	...
	public void submitChoice(@BindingParam("comp") Window x) throws Exception {
		...
		String matchString = eventCondition.UpdateMatchString(eventtype,
				eventCondition);
		Sessions.getCurrent().setAttribute("specifyAppList", dataList);
		Sessions.getCurrent().setAttribute("specifyAppMatchString", matchString);
		Sessions.getCurrent().setAttribute("AppMatchString",matchString);
		x.detach();
	}
	...
	public String[] GetTargetString() {
		...
		for (int j = 0; j < dataList.getSize(); j++) {
			targetString[j] = dataList.get(j).getAppName().toString();
		}
		return targetString;
	}

	public String appNameMatch;

	public String getAppNameMatch() {
		return appNameMatch;
	}

	public void setAppNameMatch(String appNameMatch) {
		this.appNameMatch = appNameMatch;
	}

	private ListModelList<App> dataList = new ListModelList<>();

	@Command
	@NotifyChange("dataList")
	public void addItem() {
		String appNameMatch = this.appNameMatch;
		if (appNameMatch != null && (!appNameMatch.isEmpty())) {
			boolean chkPass = true;
			for (int j = 0; j < dataList.getSize(); j++) {
				if (dataList.get(j).getAppName().toString()
						.equals(appNameMatch)) {
					WebUtil.showInfoBoxWithCustomMessage("The list already contains " + appNameMatch);
					chkPass = false;
					break;
				}
			}
			if (chkPass) {
				App app = new App(appNameMatch);
				dataList.add(app);
				setAppNameMatch("");
	...
	public ListModelList<App> getDataList() {
		return dataList;
	}

	public void setDataList(ListModelList<App> dataList) {
	...
	public class App {
		...
		public void setDataBaseName(String appName) {
	...
	public void GetData() {
		...
		{
			String appNameMatch = (String) EventNodeDataValue.get("" + i);
			App app = new App(appNameMatch);
			setAppNameMatch(appNameMatch);
			addItem();
		}
	}
}
