...
public class SpecifyLoginViewModel implements
	...
	public void submitChoice(@BindingParam("comp") Window x) throws Exception {
		...
		String matchString = eventCondition.UpdateMatchString(eventtype,
				eventCondition);
		Sessions.getCurrent().setAttribute("specifyLoginList", dataList);
		Sessions.getCurrent().setAttribute("specifyLoginMatchString", matchString);
		Sessions.getCurrent().setAttribute("LoginMatchString",matchString);
		x.detach();
	}
	...
	public void addItem() {
		...
		if (loginNameMatch != null && (!loginNameMatch.isEmpty())) {
			...
			for (int j = 0; j < dataList.getSize(); j++) {
				if (dataList.get(j).getLoginName().toString()
						.equals(loginNameMatch)) {
	...
	public void GetData() {
		...
		{
			String loginNameMatch = (String) EventNodeDataValue.get("" + i);
			Login login = new Login(loginNameMatch);
			setLoginNameMatch(loginNameMatch);
			addItem();
		}
	}
}
