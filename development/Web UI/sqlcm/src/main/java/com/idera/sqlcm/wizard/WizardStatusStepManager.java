...
public class WizardStatusStepManager {

	private static final Logger logger = Logger
			.getLogger(WizardStatusStepManager.class);
	public static final String STEP_VM_ID = "step_vm";

	private AbstractStatusWizardViewModel mParentWizardViewModel;

	private LinkedList<IStatusStep> mStepsStack; // used to handle back button.
	private HashMap<String, IStatusStep> mRegisteredSteps;
	private List<String> mZulStepList = new ArrayList<>(10);

	WizardStatusStepManager(AbstractStatusWizardViewModel parentWizardViewModel) {
	...
	public WizardStatusStepManager registerStep(String zulPath) {
		if (zulPath == null) {
	...
	public void initSteps(Panelchildren mainPanelChildren) {
		...
		for (String zulPath : mZulStepList) {
			...
			IStatusStep stepViewModel;
			try {
				stepViewModel = (IStatusStep) viewModelStepObj;
	...
	public IStatusStep getFirst() {
		return mStepsStack.peekFirst();
	}

	IStatusStep next() {
		// get current step. must not be null.
		IStatusStep currentStep = mStepsStack.peekFirst();
		...
		if (nextStepZul != null) {
			...
			IStatusStep stepToDisplay = mRegisteredSteps.get(nextStepZul);
	...
	IStatusStep getCurrentStep() {
		return mStepsStack.peekFirst();
	}

	IStatusStep getStepOrException(Class<? extends IAlertStep> stepClass)
			throws NoSuchElementException {
		IStatusStep result = mRegisteredSteps.get(stepClass);
		if (result == null) {
