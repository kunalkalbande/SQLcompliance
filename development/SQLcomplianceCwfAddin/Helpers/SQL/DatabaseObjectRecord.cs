...
{
    /// <summary>
    /// Class was created based on the Idera.SQLcompliance.Application.GUI.Helper.DatabaseObjectRecord class
    /// </summary>
    public class DatabaseObjectRecord
    {
        ...
        // GetUserTables
        //-----------------------------------------------------------------------------
        static public ICollection GetUserTables(int dbId, SqlConnection connection)
        {
            string where = String.Format("dbId={0} AND objectType=0", dbId);
            return GetDatabaseObjects(where, connection);
        }

        //-----------------------------------------------------------------------------
        // DeleteUserTables
        //-----------------------------------------------------------------------------
        static public bool DeleteUserTables(int dbId, SqlTransaction transaction, SqlConnection connection )
        {
            bool retval = false;
        ...
        // CreateUserTables
        //-----------------------------------------------------------------------------
        static public bool CreateUserTables(ICollection newItems, int dbId, SqlTransaction transaction, SqlConnection connection)
        {
            // no existing rows 
            return UpdateUserTables(newItems, 0, dbId, transaction, connection); 
        }

        //-----------------------------------------------------------------------------
        // UpdateUserTables
        //-----------------------------------------------------------------------------
        static public bool UpdateUserTables( ICollection newItems, int oldCount, int dbId, SqlTransaction transaction, SqlConnection connection )
        {
            bool retval = false;
            int expectedRows = 0;
            ...
            try
            {
                ...
                // Insert Tables
                foreach (DatabaseObjectRecord dbo in newItems)
                {
                    if (dbo.dbId == -1)
                    {
                        dbo.dbId = dbId;
                ...
                // Execute 
                //---------
                SqlCommand cmd = new SqlCommand(s.ToString(), connection);
                if (transaction != null)
                {
                    cmd.Transaction = transaction;
                }
        ...
        // GetDatabaseObjects
        //-----------------------------------------------------------------------------
        static public ICollection GetDatabaseObjects(string whereClause, SqlConnection connection)
        {
            IList dboList;
