...
public class EventFiltersGridViewReport extends CommonGridViewReport {
    ...
    public EventFiltersGridViewReport(String reportTitle, String reportSubTitle, String reportName) {
    ...
    public void setDataMapForListInstance(ListModelList<CMEntity> modelList) {
        ...
        List<CMEventFilters> alertsModel = new ListModelList<>();
        for (CMEntity entity : modelList) {
            if (entity instanceof CMEventFilters) {
                alertsModel.add((CMEventFilters) entity);
            }
          }

        for (CMEventFilters alert :alertsModel) {
            ...
            if (EventFiltersColumns.FILTER.isVisible()){
			 addValidValueToMap(alert.getName(), EventFiltersColumns.FILTER.getColumnId(),EventFiltersColumns.FILTER.getLabelKey(), tmpMap, addColumn);
            }
			if (EventFiltersColumns.INSTANCE.isVisible()){
				 addValidValueToMap(alert.getTargetInstances(), EventFiltersColumns.INSTANCE.getColumnId(),EventFiltersColumns.INSTANCE.getLabelKey(), tmpMap, addColumn);
			 }
			if (EventFiltersColumns.STATUS.isVisible()){
				addValidValueToMap(alert.getEnabled() == false ?"Disable" : "Enable", EventFiltersColumns.STATUS.getColumnId(),EventFiltersColumns.STATUS.getLabelKey(), tmpMap, addColumn);
			}
			if (EventFiltersColumns.DESCRIPTION.isVisible()){
				addValidValueToMap(alert.getDescription(), EventFiltersColumns.DESCRIPTION.getColumnId(),EventFiltersColumns.DESCRIPTION.getLabelKey(), tmpMap, addColumn);
			}
