...
namespace Idera.Common.ReportsInstaller.Model
{
	...
	public class RsModel : IRsModel
	{
		...
		public RsModel(IViewAccessAdapter viewAdapter, string databaseName,
			...
			string xmlReportsName, string reportsXPathExpression, 
		...
		{
			ErrorLog.Singleton.LogSuccess("viewAdapter");
			_viewAdapter = viewAdapter;
			ErrorLog.Singleton.LogSuccess("databaseName");
			_databaseName = databaseName;
			ErrorLog.Singleton.LogSuccess("tableName");
			_tableName = tableName;
			ErrorLog.Singleton.LogSuccess("columns");
			_columns = columns;
			ErrorLog.Singleton.LogSuccess("columnTypes");
			_columnTypes = columnTypes;
			ErrorLog.Singleton.LogSuccess("regkey");
			_regkey = regkey;
			ErrorLog.Singleton.LogSuccess("xmlReportsName");
			_xmlReportsName = xmlReportsName;
			ErrorLog.Singleton.LogSuccess("reportsXPathExpression");
			_reportsXPathExpression = reportsXPathExpression;
			ErrorLog.Singleton.LogSuccess("startMenuPath");
			_startMenuPath = startMenuPath;
			ErrorLog.Singleton.LogSuccess("shortcutFile");
			_shortcutFile = shortcutFile;
			ErrorLog.Singleton.LogSuccess("rsFac ssl");
			_rsFac = new SslProxyFactory(viewAdapter);
			ErrorLog.Singleton.LogSuccess("requiredPermissions");
			_requiredPermissions = requiredPermissions;
			ErrorLog.Singleton.LogSuccess("parentPath");
			_parentPath = parentPath;
			ErrorLog.Singleton.LogSuccess("folderDescription");
			_folderDescription = folderDescription;
			ErrorLog.Singleton.LogSuccess("dsName");
			_dsName = dsName;
			ErrorLog.Singleton.LogSuccess("dsDescription");
			_dsDescription = dsDescription;
			ErrorLog.Singleton.LogSuccess("reportingServicesTemplate");
			_reportingServicesTemplate = reportingServicesTemplate;
			ErrorLog.Singleton.LogSuccess("mainTemplate");
			_mainTemplate = mainTemplate;
			ErrorLog.Singleton.LogSuccess("reportsTemplate");
			_reportsTemplate = reportsTemplate;
			ErrorLog.Singleton.LogSuccess("dictReportFile");

			_dictReportFile = new Hashtable();
			ErrorLog.Singleton.LogSuccess("dictReportDes");
			_dictReportDes = new Hashtable();
			ErrorLog.Singleton.LogSuccess("dictReportInstalled");
			_dictReportInstalled = new Hashtable();
			ErrorLog.Singleton.LogSuccess("rsFac proxy");
			_rsFac = ProxyFactory.Singleton;
			ErrorLog.Singleton.LogSuccess("RsModel complete");
		}
			...
			string folderPath)
		{
			...
			foreach (CatalogItem ci in items)
			{
				...
				{
					if (File.Exists((string)_dictReportFile[ci.Name]))
					{
			...
			{
				if (File.Exists((string)_dictReportFile[key]))
				{
...
//				stream.Close();
		...
		{
			bool check = File.Exists(_xmlReportsName);
			if (check == false)
			{
