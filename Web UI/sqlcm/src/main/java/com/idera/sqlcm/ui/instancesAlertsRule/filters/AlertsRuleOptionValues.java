...
public enum AlertsRuleOptionValues {
    ...
    private static Map<Integer, AlertsRuleOptionValues> lookup = new HashMap<Integer, AlertsRuleOptionValues>();
    static {
        for (AlertsRuleOptionValues status : AlertsRuleOptionValues.values()) {
            lookup.put(status.intValue, status);
        }
    }

    AlertsRuleOptionValues(String id, String value, int intValue, String label, String imageURL) {
        this.id = id;
        this.value = value;
