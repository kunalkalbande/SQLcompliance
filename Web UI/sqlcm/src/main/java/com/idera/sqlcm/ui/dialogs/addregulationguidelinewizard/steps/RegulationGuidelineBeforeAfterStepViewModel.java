...
import com.idera.sqlcm.ui.dialogs.addserverwizard.steps.RegulationGuidelineBeforeAfterStepViewModel.RootNodeData;

public class RegulationGuidelineBeforeAfterStepViewModel extends RegulationGuidelineAddWizardStepBase {

    public static final String ZUL_PATH = "~./sqlcm/dialogs/regulationguidelinewizard/steps/before-after-step.zul";
    ...
    public void onAddBtnClick() {
        ...
        boolean dmlEnabled = false;
        try {
        	CMInstance instance = InstancesFacade.getInstanceDetails(getInstance().getId());
			CMAuditedDatabase database = DatabasesFacade.getDatabaseDetails(
					Long.toString(instance.getId()),Long.toString(db.getId()));
			dmlEnabled = getParentWizardViewModel().getWizardEntity().getAuditedActivities().isAuditDML();
			BeforeAfterConfigureViewModel.showDialog(instance,database,db,dmlEnabled,beforeAfterConfigureListener);
    ...
    private class BeforeAfterConfigureListener implements BeforeAfterConfigureViewModel.DialogListener {
		...
		public void onCloseBeforeAfterConfigureDialog(
			    ...
			    {
			    	if(cmStringCMTableEntity.getValue().getDatabaseId() == (Long)Sessions.getCurrent().getAttribute("dbId")){
			    		tableListModel.add(cmStringCMTableEntity.getValue());
			    	}
			    }
			    if(data instanceof CMDatabase){
			    	((CMDatabase)data).setBeforeAfterTableData(tableListModel);
			    }
		 }
    }
