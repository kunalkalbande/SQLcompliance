...
public class InstanceAlertsViewModel extends BaseViewModel {

	private static final Logger logger = Logger.getLogger(InstanceAlertsViewModel.class);
	...
	protected void loadData() {
		try {
			...
			if (this.entities == null) {
				this.entities = new ListModelList<InstanceAlert>();
			} else {
				this.entities.clear();
			}
			// initialize the alert types
			if (this.entityType == EntityType.All) {
				this.alertType = RuleType.All;
			}
			this.entities.addAll(InstancesFacade.getInstancesAlerts(this.alertType));
			logger.info("Entities loaded: " + this.entities.size());
		} catch (Exception ex) {
			logger.error("Exception in fetching " + entityType.name() +
					" instances alerts", ex);
		}
	}
