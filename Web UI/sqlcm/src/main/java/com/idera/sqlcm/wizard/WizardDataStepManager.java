...
public class WizardDataStepManager {

	private static final Logger logger = Logger
			.getLogger(WizardDataStepManager.class);
	public static final String STEP_VM_ID = "step_vm";

	private AbstractDataAlertWizardViewModel mParentWizardViewModel;

	private LinkedList<IDataStep> mStepsStack; // used to handle back button.
	private HashMap<String, IDataStep> mRegisteredSteps;
	private List<String> mZulStepList = new ArrayList<>(10);

	WizardDataStepManager(AbstractDataAlertWizardViewModel parentWizardViewModel) {
	...
	public WizardDataStepManager registerStep(String zulPath) {
		if (zulPath == null) {
	...
	public void initSteps(Panelchildren mainPanelChildren) {
		...
		for (String zulPath : mZulStepList) {
			...
			IDataStep stepViewModel;
			try {
				stepViewModel = (IDataStep) viewModelStepObj;
			} catch (ClassCastException e) {
				throw new RuntimeException(String.format(
						" Step viewModel class in '%s' must be heir of '%s' ",
						zulPath, IDataStep.class.getName()));
			}
	...
	public IDataStep getFirst() {
		return mStepsStack.peekFirst();
	}

	IDataStep next() {
		// get current step. must not be null.
		IDataStep currentStep = mStepsStack.peekFirst();
		...
		if (nextStepZul != null) {
			...
			IDataStep stepToDisplay = mRegisteredSteps.get(nextStepZul);
	...
	IDataStep getCurrentStep() {
		return mStepsStack.peekFirst();
	}

	IDataStep getStepOrException(Class<? extends IDataStep> stepClass)
			throws NoSuchElementException {
		IDataStep result = mRegisteredSteps.get(stepClass);
		if (result == null) {
